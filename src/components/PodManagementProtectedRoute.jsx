...
function PodManagementLoadingScreen() {
  return (
    <div className="auth-state-screen" role="status" aria-live="polite">
      <LoaderCircle className="auth-spinner" aria-hidden="true" />
      <p>Checking Pod Management workspace access...</p>
    </div>
  );
}

export default function PodManagementProtectedRoute() {
  ...
  if (loading) {
    return <PodManagementLoadingScreen />;
  }
  ...
  if (authorizationError || !isActiveAppUser || !hasPodManagementAccess) {
    let message =
      "Your account does not have access to the Pod Management workspace.";
    ...
    return (
      <div className="auth-state-screen">
        <section className="auth-access-card" aria-labelledby="pod-access-title">
          <ShieldAlert aria-hidden="true" />
          <h1 id="pod-access-title">Access denied</h1>
          <p>{message}</p>
