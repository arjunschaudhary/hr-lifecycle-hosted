...
import { isGmailProviderError, sendEmailWithPdfAttachment } from "../_shared/gmailProvider.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    try {
      const gmail = await sendEmailWithPdfAttachment(candidate.email, { subject: `${name} – Jarurat Care Foundation`, text: `Dear ${candidate.full_name},\n\nYour ${name} is attached.`, html: `<p>Dear ${candidate.full_name},</p><p>Your ${name} is attached.</p>` }, { filename: `${template.templateKey}.pdf`, contentType: "application/pdf", bytes: pdf });
