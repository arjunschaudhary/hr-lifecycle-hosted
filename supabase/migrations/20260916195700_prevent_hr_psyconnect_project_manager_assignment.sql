    ...
    if v_lead_type = 'TECH_LEAD'
       and exists (
        ...
        raise exception using
            errcode = 'P0001',
            message = 'An active HR Psyconnect user cannot be assigned as Project Manager.';
    end if;
...
comment on function public.assign_candidate_lead_to_pod(uuid, uuid, text, date) is
    'Atomically adds a Pod Lead or Tech Lead role and matching pod membership to an existing active candidate portal user without replacing the Candidate role or changing the portal mapping. Active HR_SITE_CONNECT users cannot be assigned as Tech Leads.';
