...
-- Add other_leave_type_reason column to leave_requests if not exists
alter table public.leave_requests add column if not exists other_leave_type_reason text;

-- Drop the old 5-parameter signature to prevent overloading issues
drop function if exists public.submit_current_candidate_leave_request(text, date, date, text, text);

-- Recreate or update submit_current_candidate_leave_request with 6 parameters (including p_other_leave_type_reason)
create or replace function public.submit_current_candidate_leave_request(
    ...
    p_other_leave_type_reason text default null
)
returns jsonb
language plpgsql
security definer
...
declare
    ...
    v_other_leave_type_reason text;
...
begin
    ...
    -- Validate past dates using Asia/Kolkata timezone
    v_today := (timezone('Asia/Kolkata'::text, v_now)::date);
    ...
    v_other_leave_type_reason := nullif(btrim(p_other_leave_type_reason), '');
    ...
    if v_leave_type = 'Other' and v_other_leave_type_reason is null then
        raise exception 'Specify reason is required for other leave type.'
            using errcode = '23514';
    end if;
    ...
    if v_supporting_document is not null then
        -- Allow either standard HTTP/HTTPS URLs (for backward compatibility) OR candidate folder storage path matching current candidate ID
    ...
    return jsonb_build_object(
        ...
        'otherLeaveTypeReason', v_request.other_leave_type_reason,
...
comment on function public.submit_current_candidate_leave_request(text, date, date, text, text, text) is
    'Creates one pending leave request for the current active candidate mapping with support for 14 leave types and validation of past dates.';

revoke execute on function public.submit_current_candidate_leave_request(text, date, date, text, text, text) from public;
revoke execute on function public.submit_current_candidate_leave_request(text, date, date, text, text, text) from anon;
grant execute on function public.submit_current_candidate_leave_request(text, date, date, text, text, text) to authenticated;
grant execute on function public.submit_current_candidate_leave_request(text, date, date, text, text, text) to service_role;

-- Recreate get_current_candidate_leave_requests RPC to return other_leave_type_reason
...
comment on function public.get_current_candidate_leave_requests() is
    'Returns only the leave-request history belonging to the current active candidate mapping including other_leave_type_reason.';
...
-- Recreate leave_requests_view to include other_leave_type_reason
create or replace view public.leave_requests_view as
select
    lr.leave_request_id,
    lr.candidate_id,
    lr.mid,
