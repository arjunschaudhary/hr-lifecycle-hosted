...
comment on function public.get_candidate_lead_review(uuid) is
    'Returns one candidate-cycle Lead review with its candidate, cycle, pod, daily-scoring readiness, review-window state, and reviewer name. Read access is limited to active authorized HR dashboard users or active Pod Lead or Tech Lead users with matching pod authority; reviewer email addresses are not exposed.';
...
comment on function public.save_candidate_lead_review(
    ...
    'Creates or updates one partial Lead review draft, or submits one complete Lead review, for an active authorized Pod Lead or Tech Lead user with matching pod authority. Submitted reviews are immutable; submission refreshes the stored review summary and candidate result status and records a permanent activity log atomically.';
