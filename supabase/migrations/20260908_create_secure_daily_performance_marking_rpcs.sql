...
comment on function public.get_candidate_daily_performance_entries(uuid) is
    'Returns the complete inclusive evaluation-date marking grid for one candidate performance cycle, including scorable-day eligibility and any existing daily marks. Access is read-only and limited to active authorized HR users; reviewer email addresses are not exposed.';

revoke execute on function public.get_candidate_daily_performance_entries(uuid) from public;
revoke execute on function public.get_candidate_daily_performance_entries(uuid) from anon;
grant execute on function public.get_candidate_daily_performance_entries(uuid) to authenticated;
grant execute on function public.get_candidate_daily_performance_entries(uuid) to service_role;
