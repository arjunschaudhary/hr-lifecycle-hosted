...
-- Update the candidate portal summary RPC to expose:
--   signedOffer.verificationNotes  (HR rejection notes; only populated for MISMATCH_REVIEW)
--   signedOffer.canResubmit        (true only when lifecycle is in MISMATCH_REVIEW)
--
-- All existing signedOffer fields and their behavior are preserved unchanged.
-- This is a safe CREATE OR REPLACE of the existing function.
...
begin
    ...
    -- canSubmit: unchanged from original — only true when lifecycle is ACTIVE
    -- and no in-progress verification row exists.
    ...
    -- canResubmit: true only when the candidate's lifecycle is MISMATCH_REVIEW
    -- and there is an active MISMATCH_REVIEW verification row.
    ...
    return jsonb_build_object(
        ...
        'signedOffer', jsonb_build_object(
            ...
            -- verificationNotes only exposed when the status is MISMATCH_REVIEW
...
comment on function public.get_current_candidate_portal_summary() is
    'Returns only the safe portal summary for the current authenticated candidate. The candidate is resolved through current_candidate_id() and no other candidate data is exposed. Exposes canResubmit and verificationNotes (only for MISMATCH_REVIEW) in the signedOffer block.';
