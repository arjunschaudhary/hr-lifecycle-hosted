...
comment on function public.assign_candidate_lead_to_pod(uuid, uuid, text, date) is
    'Atomically adds a Pod Lead or eligible Project Manager role and matching pod membership to an existing active candidate portal user without replacing the Candidate role or changing candidate pod membership.';
